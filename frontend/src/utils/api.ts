...
function normalizeSession(apiData: ApiSession, characterData?: ApiCharacter): ChatSession {
  ...
  return {
    ...
    character,
    createdAt: apiData.created_at, // Map backend snake_case to frontend camelCase
    updatedAt: apiData.updated_at, // Map backend snake_case to frontend camelCase
  };
}
...
interface SendMessageRequest {
  ...
  file_uri?: string;
  // New optional fields for creating session
  world_time?: string;
  user_persona?: string;
  enable_web_search?: boolean;
  output_language?: string;
...
class ApiService {
  ...
  async sendMessage(data: SendMessageRequest): Promise<ApiResponse<{ ai_message: ApiMessage; chat_session_id?: string }>> {
    const requestData = {
      ...
      // Include optional settings for new session creation
