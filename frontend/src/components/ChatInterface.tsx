...
import { apiService, getAuthToken, removeAuthToken, SendMessageRequest, StreamMessageEvent } from '@/utils/api';
import { AttachmentKind, getAttachmentSupport } from '@/utils/modelCapabilities';
import { User, LogOut, Cpu, Globe, FolderTree } from 'lucide-react';
...
export default function ChatInterface({
  ...
  useEffect(() => {
    ...
    if (token) {
      setCurrentUser(copy.chat.authenticatedUser);
    }
  }, [copy.chat.authenticatedUser]);
  ...
  const handleSendMessage = async (userInput: string, attachments: PendingAttachment[] = []) => {
    ...
    const currentUserLabel = currentUser || copy.chat.you;
  ...
  return (
    <div className="flex h-full flex-col bg-[linear-gradient(180deg,#f8fbff_0%,#eef4f8_52%,#f4efe8_100%)]">
      <header className="border-b border-slate-200/70 bg-white/75 px-6 py-5 backdrop-blur-xl">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-start gap-4">
            <div className="flex h-14 w-14 flex-shrink-0 items-center justify-center overflow-hidden rounded-[1.25rem] bg-gradient-to-br from-sky-100 via-cyan-50 to-amber-50 shadow-sm ring-1 ring-white/70">
              {character?.avatarUrl ? (
            ...
            <div className="min-w-0 space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-full bg-slate-900 px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.16em] text-white">
                  {copy.chat.directChat}
                </span>
                {character?.affiliation && (
                  <span className="rounded-full bg-white/80 px-2.5 py-1 text-xs text-slate-600 ring-1 ring-slate-200">
                    {character.affiliation}
                  </span>
                )}
              </div>
              <div>
                <h2 className="text-xl font-semibold tracking-tight text-slate-900">{character?.name || copy.chat.loadingCharacter}</h2>
                <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-600">{(() => {
                  const text = character?.description || copy.chat.assistantFallback;
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
                <div className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-1">
                  <Cpu size={12} />
                  <span>
                    {activeModelConfig ? `${activeModelConfig.name} / ${activeModelConfig.modelName}` : copy.chat.systemDefaultModel}
                    {chatSession?.lastResponseLatencyMs ? ` / ${(chatSession.lastResponseLatencyMs / 1000).toFixed(1)}s` : ''}
                    {' / Stream'}
                  </span>
                </div>
                {chatSession && (
                  <div className="inline-flex items-center gap-1.5 rounded-full bg-white/80 px-2.5 py-1 ring-1 ring-slate-200">
                    <span className="h-2 w-2 rounded-full bg-emerald-500" />
                    <span>{copy.chat.sessionActive}</span>
                  </div>
                )}
                <div className="inline-flex items-center rounded-full bg-white/80 px-2.5 py-1 ring-1 ring-slate-200">
                  {copy.chat.fileSupport(attachmentSupport.image, attachmentSupport.video)}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 lg:justify-end">
            {currentUser && (
              <div className="flex items-center gap-2 rounded-full bg-white/80 px-3 py-2 text-sm text-slate-600 ring-1 ring-slate-200">
                <User size={16} />
            ...
            {currentUser && (
              <button
                ...
                className="rounded-full bg-white/80 p-2.5 text-slate-500 ring-1 ring-slate-200 transition-colors hover:bg-white hover:text-slate-900"
                title={copy.chat.logout}
              >
                <LogOut size={18} />
              </button>
            )}
      ...
      {showResearchPanel && (
        <ResearchPanel latestAssistantMessage={latestResearchMessage} />
      )}
      ...
      <div className="flex-1 overflow-hidden px-4 pb-4 pt-4 md:px-6 md:pb-6">
        <ImmersiveChatWindow
          ...
          currentUserLabel={currentUser || copy.chat.you}
      ...
      {character && (
        <div className="hidden border-t border-slate-200/70 bg-white/70 px-6 py-4 backdrop-blur xl:block">
