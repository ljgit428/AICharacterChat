...
export default function LoginModal({ isOpen, onClose, onLogin }: LoginModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(messages.auth.unexpectedError);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
        <div className="p-6">
          <h2 className="text-2xl font-bold mb-4">
            {isLogin ? messages.auth.login : messages.auth.register}
          </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex space-x-2">
              ...
              >
                {isLoading ? messages.auth.loading : (isLogin ? messages.auth.login : messages.auth.register)}
              </button>
              <button
                type="button"
          ...
          <div className="mt-4 text-center">
            ...
            >
              {isLogin ? messages.auth.toggleToRegister : messages.auth.toggleToLogin}
            </button>
          </div>
