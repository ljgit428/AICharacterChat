...
| L0/L1/L2 分层：**分阶段吸收效果，不引入术语**（2026-08-23 修正：作者原意是"不了解用法与效果"，并非拒绝） | 三层的*效果*拆解——L0 核心常驻＝§3.2 优先级注入 + §4 关系分区（本期实现）；L1 近期状态＝会话历史（已有）；L2 长尾语义检索＝唯一真实缺口，但仅在记忆达数百条后才有可感知收益。**触发条件（满足其一即立项 L2 检索）**：「关系+里程碑之外」条目数 > 100，或 narrative 持续顶满注入预算。届时实现方式为 pgvector + embedding（候选：BGE-M3 本地量化版 or API）。在此之前维持"全量+裁剪"注入；L0/L1/L2 命名本身不进入代码与 UI |
