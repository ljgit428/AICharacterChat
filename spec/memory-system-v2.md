...
> Status: **部分实现 — 阶段 1–3 已随 v0.1.2 落地。** v1（`spec/memory-system-spec.md`）
> 已落地的部分保持不变；本文档只描述增量改动，冲突时以本文档为准。
>
...
> 结论性取舍记录在 §7「明确不做」。
>
> **版本标注：** 起草于 v0.1.2 开发分支（提交 `14f73b9`，2026-08-23）。
> §1 问题清单的 P0/P1 修复与阶段 1–3 已随 v0.1.2 落地：可靠性
> （`2a2383a`）、关系与里程碑成长（`36914c7`）、自然聊天队列与记忆可见性
> （`5820aad`）；其余条目（如定期整理、衰减设计位）仍待实施。
